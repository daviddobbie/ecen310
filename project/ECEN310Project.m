...
function [capacity] = generateFM(macro_userDens, femto_Dens, R , R_outRange, r_femto, ...
    ...
    Nusr = cast(2e4/(N_femto*p_activity), 'uint32');
    ...
    %P_tx_macro = db2pow(pow2db(P_tx_macro) + 10);
    ...
    for cellIdx = 1:N_femto
        ...
        for usrIdx = 1:Nusr
            ...
            if (abs(users(usrIdx)) < R) %user wtihin femto cell placed within R
                ...
                abs(other_femto_pos(cellIdx) - users);
                ...
                %{
                % capping femto received poewer for inteference
                for idx = 1:N_active
                    if pow2db(I_femto_individ(idx)) - NodB > SNR_maxdB
                        I_femto_individ(idx) = db2pow(SNR_maxdB + NodB);
                    end 
                end
                %}
                ...
                %I_femto_individ = P_tx_femto_cell .* other_femto_dists .^(-3) ...
                %    .* h  .* (L) ./ ((1+ femto_user_inside(usrIdx))*(db2pow(wallLossdB)));
                ...
                L = db2pow((sigma_sfdB)*randn(1,1));
                h = (  abs(    sqrt(2) * ( randn(1,1) + 1j*randn(1,1) )    )  ).^2;
                ...
                macro_power = [macro_power P_rx_femto_user_longterm];
